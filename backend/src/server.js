...
function getAvailableModelsCached() {
    ...
    const models = (0, aiService_1.getAvailableModels)();
    cachedModels = models;
    lastModelCheck = now;
    return models;
}
...
function checkRateLimitInMemory(uid) {
    const now = Date.now();
...
function isValidationError(message) {
    return (message.includes('cannot be empty') ||
...
app.get('/health', (_req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            limits: { maxPromptLength: aiService_1.AI_PROMPT_MAX_LENGTH },
            version: '1.0.0',
        });
    }
    catch (error) {
...
app.post('/api/resolve-doubt', authMiddleware, checkRateLimit, async (req, res) => {
    ...
    if (typeof question === 'string' && question.length > aiService_1.AI_PROMPT_MAX_LENGTH) {
        return res.status(400).json({ error: `Question exceeds maximum length (${aiService_1.AI_PROMPT_MAX_LENGTH} characters)` });
    }
    const modelType = (model === 'mistral' ? 'mistral' : 'llama');
    try {
        const resolution = await aiService_1.aiService.resolveDoubt(question, context || 'General Education', curriculumContext, modelType);
        if (res.headersSent)
            return;
...
app.post('/api/generate-content', authMiddleware, checkRateLimit, async (req, res) => {
    ...
    if (typeof prompt === 'string' && prompt.length > aiService_1.AI_PROMPT_MAX_LENGTH) {
        return res.status(400).json({ error: `Prompt exceeds maximum length (${aiService_1.AI_PROMPT_MAX_LENGTH} characters)` });
    }
    const modelType = (model === 'mistral' ? 'mistral' : 'llama');
    try {
        const content = await aiService_1.aiService.generateResponse(prompt, modelType);
        if (res.headersSent)
            return;
        const latency = Date.now() - startTime;
...
app.post('/api/generate-teaching-content', authMiddleware, checkRateLimit, async (req, res) => {
    ...
    if (fullPrompt && typeof fullPrompt === 'string' && fullPrompt.trim().length > 0) {
        ...
        try {
            const content = await aiService_1.aiService.generateTeachingContentFromPrompt(fullPrompt.trim(), modelType);
            if (res.headersSent)
                return;
            const latency = Date.now() - startTime;
    ...
    try {
        const content = await aiService_1.aiService.generateTeachingContent(topic, curriculumContext, modelType);
        if (res.headersSent)
            return;
        const latency = Date.now() - startTime;
...
app.post('/api/generate-quiz', authMiddleware, checkRateLimit, async (req, res) => {
    ...
    const modelType = (model === 'mistral' ? 'mistral' : 'llama');
    try {
        const quiz = await aiService_1.aiService.generateQuiz(topic, context || 'General Education', curriculumContext, modelType);
        if (res.headersSent)
            return;
        const latency = Date.now() - startTime;
